...
void move_and_log(cv::VideoCapture inputVideo, string type, cv::Vec2f location, float speed)
{
  ...
  while (true)
  {
    ...
    if (yprs.size() > 0)
    {
      ...
      {
        det_loc = cv::Vec3f(tvecs[i][0] * 100,tvecs[i][1] * 100,tvecs[i][2] * 100);
        real_loc = cv::Vec2f((controller.loc[0]) / 10,((controller.loc[1]+76+cam_dist) / 10));
        float distDiff = pow(pow(det_loc[0] - real_loc[0],2) + pow(det_loc[2] - real_loc[1],2),.5);

        std::cout << "x " << det_loc[0] << "(" << real_loc[0] << ")" << " ,y " << det_loc[1]
                  << " ,z " << det_loc[2] << "(" << real_loc[1] << ")" << "(" << distDiff << ")"
                  << std::endl;
...
int main(void)
{
  ...
  // setup moves
  controller.move(cv::Vec2f(40, 0), 10000);
  ...
  cv::Vec2f moveLoc(-40, 40);
  cv::Vec2f moveLoc2(0, -40);

  cv::Vec3f moveRot(0, 0, 0);
  double speed = 10000; // mm/min
